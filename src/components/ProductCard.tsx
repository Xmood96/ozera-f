  ...
  return (
    <>
      ...
      >
        ...
        {/* Product Content */}
        <div className={`product-content transition-all duration-300 ${
          isExpanded ? "p-8" : "p-6"
        }`}>
          {/* Product Name */}
          <h3 className={`product-name font-bold text-base-content mb-3 transition-all duration-300 ${
          ...
          {/* Product Description */}
          <p className={`product-description text-base-content/80 mb-6 transition-all duration-300 leading-relaxed ${
            ...
            <div className="expanded-content space-y-6 animate-fadeIn">
              ...
              {product.benefits && product.benefits.length > 0 && (
                <div className="benefits-section">
                  <div className="flex items-center gap-2 mb-4">
                    ...
                    <h4 className="text-lg font-semibold text-base-content">الفوائد</h4>
                  </div>
                  <div className="grid gap-2">
                    {benefitsToShow.map((benefit, index) => (
                      <div key={index} className="flex items-start gap-3 p-3 bg-success/5 rounded-xl">
                        <span className="text-success/60 mt-1">•</span>
                        <span className="text-sm text-base-content/90">{benefit}</span>
                      </div>
                    ))}
                  </div>
                  {product.benefits.length > 3 && (
                    ...
                    >
                      {showAllBenefits ? "عرض أقل" : `عرض ${product.benefits.length - 3} فوائد أخرى`}
                    </button>
                  )}
                </div>
              )}
              ...
              {product.usageInstructions && (
                <div className="usage-section">
                  <div className="flex items-center gap-2 mb-4">
                    ...
                    <h4 className="text-lg font-semibold text-base-content">طريقة الاستخدام</h4>
                  </div>
              ...
              {product.ingredients && product.ingredients.length > 0 && (
                <div className="ingredients-section">
                  <div className="flex items-center gap-2 mb-4">
                    ...
                    <h4 className="text-lg font-semibold text-base-content">المكونات</h4>
                  </div>
                  ...
                  {product.ingredients.length > 3 && (
                    ...
                    >
                      {showAllIngredients ? "عرض أقل" : `عرض ${product.ingredients.length - 3} مكونات أخرى`}
                    </button>
                  )}
                </div>
              )}
              ...
              <div className="quantity-section">
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <h4 className="text-lg font-semibold text-base-content">الكمية</h4>
                </div>
          ...
          {/* Product Footer */}
          <div className={`product-footer flex justify-between items-center gap-4 ${
            ...
            {isExpanded ? (
              <div className="expanded-actions flex items-center gap-3">
                ...
                <button
                  ...
                  className="btn-add-to-cart btn btn-primary px-8 py-3 rounded-2xl transition-all duration-300 hover:scale-105 hover:shadow-lg font-semibold text-base-content border-0 shadow-primary/25"
                  aria-label={isInCart ? `تحديث ${product.name}` : `إضافة ${product.name} إلى السلة`}
                >
                  <span className="flex items-center gap-2">
                    {isInCart ? "تحديث" : "إضافة"}
                    <span className="badge badge-primary-content bg-primary/20 text-primary px-2 py-1 rounded-full text-xs">
                      ×{quantity}
                    </span>
                  </span>
                </button>
